...
public class ResizeImage {
	...
	public static BufferedImage resizeImage(BufferedImage image, double ratio) {
		return resizeImage(image, (int) (image.getWidth() * ratio + 0.5),
				(int) (image.getHeight() * ratio + 0.5));
	}

	public static BufferedImage resizeImage(BufferedImage image, double xRatio,
			double yRatio) {
		return resizeImage(image, (int) (image.getWidth() * xRatio + 0.5),
				(int) (image.getHeight() * yRatio + 0.5));
	}

	public static BufferedImage resizeImage(BufferedImage image, int width,
			int height) {
		Double xFactor = width * 1.00 / image.getWidth();
		Double yFactor = height * 1.00 / image.getHeight();
	 ...
	 * @param picPath
	 */
	public static void thumbnails(HttpSession session,String picPath){
		if(!StringUtils.isEmpty(picPath)){	
			File fileTemp = new File(session.getServletContext().getRealPath("").concat(picPath));
			if (fileTemp.exists() && !fileTemp.isDirectory()) {
				try {
					BufferedImage image = ImageIO.read(fileTemp);
					ImageIO.write(resizeImage(image, 300, 210), "jpg", fileTemp);
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
	...
	public static void main(String[] args) throws Exception {
		BufferedImage image = ImageIO.read(new File("d:/iphone_img01.jpg"));
		ImageIO.write(resizeImage(image, 100, 100), "jpg", new File(
				"d:/image2_small.jpg"));
	}
}